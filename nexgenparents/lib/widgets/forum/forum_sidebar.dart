            ...
            children: [
              Row(
                children: [
                  ...
                  Text(l10n?.forumSidebarFollowedTopics ?? 'Temas Seguidos', style: TextStyle(fontWeight: FontWeight.bold, color: textColor, fontSize: 14)),
                ],
              ),
              const SizedBox(height: 12),
              _SidebarTopicItem(title: l10n?.forumSidebarTopic1Title ?? '¿Límites de tiempo para niños de 10 años?', subtitle: l10n?.forumSidebarTopic1Subtitle ?? '24 respuestas nuevas hoy', isDark: isDark),
              _SidebarTopicItem(title: l10n?.forumSidebarTopic2Title ?? 'Los mejores juegos educativos en Switch', subtitle: l10n?.forumSidebarTopic2Subtitle ?? '15 respuestas nuevas hoy', isDark: isDark),
              _SidebarTopicItem(title: l10n?.forumSidebarTopic3Title ?? 'Cómo gestionar la seguridad en chats online', subtitle: l10n?.forumSidebarTopic3Subtitle ?? '8 respuestas nuevas hoy', isDark: isDark, showDivider: false),
            ],
          ),
        ),
        const SizedBox(height: 12),
            ...
            children: [
              Row(
                children: [
                  Container(width: 4, height: 18, decoration: BoxDecoration(color: const Color(0xFF8B5CF6), borderRadius: BorderRadius.circular(2))),
                  const SizedBox(width: 8),
                  Text(l10n?.forumSidebarRepliesToYou ?? 'Respuestas para ti', style: TextStyle(fontWeight: FontWeight.bold, color: textColor, fontSize: 14)),
                ],
              ),
              const SizedBox(height: 12),
              _SidebarReplyItem(avatar: 'S', name: 'Sarah M.', link: l10n?.forumSidebarReply1Link ?? 'Seguridad en Fortnite', action: l10n?.forumSidebarReply1Action ?? 'respondió a tu co...', time: l10n?.forumSidebarReply1Time ?? 'hace 2 minutos', isDark: isDark, avatarColor: const Color(0xFFEC4899)),
              _SidebarReplyItem(avatar: 'D', name: 'David K.', link: l10n?.forumSidebarReply2Link ?? 'Comparativa de consolas', action: l10n?.forumSidebarReply2Action ?? 'te etiquetó en...', time: l10n?.forumSidebarReply2Time ?? 'hace 1 hora', isDark: isDark, avatarColor: const Color(0xFF3B82F6)),
              _SidebarReplyItem(avatar: 'E', name: 'Emily R.', link: l10n?.forumSidebarReply3Link ?? 'Hilo de Bienvenida', action: l10n?.forumSidebarReply3Action ?? 'le gustó tu respue...', time: l10n?.forumSidebarReply3Time ?? 'hace 3 horas', isDark: isDark, avatarColor: const Color(0xFF10B981), showDivider: false),
            ],
          ),
        ),
        const SizedBox(height: 12),
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(l10n?.forumSidebarGlobalNews ?? 'Noticias Globales del Foro', style: TextStyle(fontWeight: FontWeight.bold, color: textColor, fontSize: 14)),
                ],
              ),
              const SizedBox(height: 12),
              _SidebarNewsItem(tag: l10n?.forumSidebarNews1Tag ?? 'Actualización', tagColor: const Color(0xFF8B5CF6), text: l10n?.forumSidebarNews1Text ?? 'Nuevas guías de control parental añadidas al Diccionario.', isDark: isDark),
              const SizedBox(height: 8),
              _SidebarNewsItem(tag: l10n?.forumSidebarNews2Tag ?? 'Evento', tagColor: const Color(0xFF3B82F6), text: l10n?.forumSidebarNews2Text ?? 'Q&A en vivo con psicólogo infantil este jueves a las 18:00.', isDark: isDark),
              const SizedBox(height: 8),
              _SidebarNewsItem(tag: l10n?.forumSidebarNews3Tag ?? 'Novedad', tagColor: const Color(0xFF10B981), text: l10n?.forumSidebarNews3Text ?? '¡El modo oscuro ya está disponible en los ajustes de usuario!', isDark: isDark, showDivider: false),
            ],
          ),
        ),
      ],
